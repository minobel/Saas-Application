...
def pricing_interval_view(request, interval):
    return HttpResponse(f"Pricing interval: {interval}")


def subscription_checkout_view(request, subscription_id):
    subscription = get_object_or_404(Subscription, id=subscription_id)
    return HttpResponse(f"Checkout for subscription: {subscription}")
